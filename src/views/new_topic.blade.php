...
<form action="{{ $route('new_topic', array('id' => $forum->id)) }}" method="POST" id="post">
    <fieldset>
        ...
        <label class="required"><strong>{{ trans('fluxbb::common.subject') }} <span>{{ trans('fluxbb::common.required') }}</span></strong><br /><input type="text" name="req_subject" class="longinput" size="80" value="{{ Input::old('req_subject', '') }}" /><br /></label>
