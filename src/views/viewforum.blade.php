...
?>
    ...
    @else
            ...
            <a href="{{ route('viewpost', array('id' => $topic->id)) }}#p{{ $topic->last_post_id }}">{{ ($topic->last_post) }}</a> <span class="byuser">{{ trans('fluxbb::common.by', array('author' => ($topic->last_poster))) }}</span>
            {{-- TODO: format_time for last_post --}}
    @endif
        </div>
    </div>
@endforeach
