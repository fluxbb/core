...
?>
    ...
    @else
            ...
            <a href="{{ $route('viewpost', array('id' => $topic->id)) }}#p{{ $topic->last_post_id }}">{{ $format_time($topic->last_post) }}</a> <span class="byuser">{{ trans('fluxbb::common.by', array('author' => ($topic->last_poster))) }}</span>
    @endif
        </div>
    </div>
@endforeach
